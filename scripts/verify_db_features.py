...
async def test_db_live():
    ...
    await conn.execute(
        ...
        doc_id, 'Database Live Verification', 'test'
    )
    ...
    await conn.execute(
        ...
        ''',
        chunk_id, doc_id, vec_str
    )
    
    # 3. Verify trigger automatically computed search_tsv
    row = await conn.fetchrow('SELECT search_tsv, (search_tsv IS NOT NULL) as has_search_tsv, (tsv IS NOT NULL) as has_tsv FROM chunks WHERE id = $1', chunk_id)
    ...
    bm25_hits = await conn.fetch('SELECT id FROM chunks WHERE search_tsv @@ plainto_tsquery($1, $2)', 'english', 'Autonomous RAG')
    print(f'BM25_SUMMARY_MATCH={len(bm25_hits) >= 1}')
    
    # 5. Test HNSW Cosine vector search
    vec_hits = await conn.fetch('SELECT id, 1 - (embedding <=> $1::vector) as similarity FROM chunks WHERE id = $2', vec_str, chunk_id)
