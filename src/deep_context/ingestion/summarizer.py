...
class ChunkSummarizer:
    ...
    def _build_prompt(
        ...
        """
        Constructs Anthropic Contextual Retrieval instruction prompt with prefilled
        assistant grounding anchor to force direct factual completion without meta-filler.
        """
        system_msg = (
            "/no_think\n"
            "You are a factual document annotator. Generate a concise 1-2 sentence context that directly describes the specific entities, main topic, and key actions or facts in this chunk to situate it within the document. "
            "Never use meta-phrases like 'The document discusses' or 'The context includes'. State facts directly."
        )
        ...
        if parent_context:
            ...
            if len(parent_clean) > 800:
                parent_snippet = f"Context: {parent_clean[:800]}...\n"
            else:
                parent_snippet = f"Context: {parent_clean}\n"

        doc_block = f"{doc_header}{sec_header}{parent_snippet}".strip()

        # Build clean situating prefix anchor
        title_part = document_title or "this document"
        sec_part = f" ({section_path})" if section_path else ""
        prefix = f"In {title_part}{sec_part}, "

        user_msg = (
            f"<document>\n{doc_block}\n</document>\n\n"
            f"<passage>\n{chunk_text.strip()[:2500]}\n</passage>\n\n"
            f"Complete the factual situating context (1-2 sentences):\n{prefix}"
        )
        ...
        try:
            ...
            for idx in range(len(prompts)):
                ...
                if "<|im_start|>assistant\n<think>\n</think>\n" in prompt_text:
                    prefix = prompt_text.split("<|im_start|>assistant\n<think>\n</think>\n")[-1]
                full_text = prefix + raw_text if not raw_text.startswith("In ") else raw_text
                summary = self._clean_and_complete_summary(full_text)
            ...
            for prompt in prompts:
                try:
                    ...
                    prefix = ""
                    if "<|im_start|>assistant\n<think>\n</think>\n" in prompt:
                        prefix = prompt.split("<|im_start|>assistant\n<think>\n</think>\n")[-1]
                    full_text = prefix + raw_text if not raw_text.startswith("In ") else raw_text
                    summary = self._clean_and_complete_summary(full_text)
                    results.append((summary, len(new_tokens)))
